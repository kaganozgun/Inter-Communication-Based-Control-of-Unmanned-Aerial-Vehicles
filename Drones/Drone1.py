...
Send_Port= 5005
Recv_Port = 5006
Server_Send_Port = 5008
Server_Recv_Port = 5007
...
class Graph:
	...
	def spiral(self,minRow,maxRow,minCol,maxCol):
		...
		while(True):
			...
			while ((i+1) <= maxRow) and ((self.Matrix[i+1][j]).visited == False) :
				try:
					data, addr = ServerRecv.recvfrom(1024)
					print(data)
					if data == "stop":
				...
				#print ("distance :",self.calculateDistance(p1.lat,p1.lon,vehicle.location.global_relative_frame.lat,vehicle.location.global_relative_frame.lon))
					time.sleep(1)
				check = check + 1
			while ((j+1) <= maxCol) and ((self.Matrix[i][j+1]).visited == False) :
				try:
					data, addr = ServerRecv.recvfrom(1024)
					print(data)
					if data == "stop":
				...
				#print ("distance :",self.calculateDistance(p1.lat,p1.lon,vehicle.location.global_relative_frame.lat,vehicle.location.global_relative_frame.lon))
					time.sleep(1)
				check = check + 1
			while ((i-1) >= minRow) and ((self.Matrix[i-1][j]).visited == False) :
				try:
					data, addr = ServerRecv.recvfrom(1024)
					print(data)
					if data == "stop":
				...
				#print ("distance :",self.calculateDistance(p1.lat,p1.lon,vehicle.location.global_relative_frame.lat,vehicle.location.global_relative_frame.lon))
					time.sleep(1)
				check = check + 1
			while ((j-1) >= minCol) and ((self.Matrix[i][j-1]).visited == False) :
				try:
					data, addr = ServerRecv.recvfrom(1024)
					print(data)
					if data == "stop":
...
DroneSend.sendto("Drone1 geldi",(UDP_IP,Send_Port))
while True:
	data, addr = DroneRecv.recvfrom(1024)
	print(data)
